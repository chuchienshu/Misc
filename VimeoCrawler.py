...
class VimeoCrawler(object):
    ...
    def getItemsFromPage(self):
        ...
        try:
            ...
            # print links, ' ---------->> links'
            #[<selenium.webdriver.remote.webelement.WebElement (session="a41f240288fae2507c712ec448e38bda", element="0.38126262764447816-2")>, <selenium.webdriver.remote.webelement.WebElement (session="a41f240288fae2507c712ec448e38bda", element="0.38126262764447816-3")>, .....]
            links = (link.get_attribute('href') for link in links)
            items = tuple(URL(link) for link in links if VIMEO in link and not link.endswith('settings'))[:self.maxItems]
            # print items, '------------->> items'
            #(URL('https://vimeo.com/channels/staffpicks/videos'), URL('https://vimeo.com/channels/music/videos'),''')
        except NoSuchElementException, e:
            self.error(e.msg)
            self.dumpPage()
            items = ()
    ...
    def getItemsFromFolder(self):
        ...
        for _ in xrange(self.maxItems) if self.maxItems is not None else count():
            ...
            numPages += 1
            # print items,'*----**********'
            #[URL('https://vimeo.com/273327920'), URL('https://vimeo.com/242514334'), ....]
            try:
                print ' nextButton /////45 ////////'
                nextButton = self.getElement('.pagination a[rel=next]')
                # WebDriverWait(self.browse, 50).until(self)
                sleep(10)
                nextButton.click()
                break
                print ' nextButton /////45689 ////////'
            except NoSuchElementException:
                break
        items = tuple(items)
        print items
